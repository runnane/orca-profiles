...
export function notLoadedPresets(
  ...
  for (;;) {
    ...
    for (const p of active) {
      ...
      if (firstLoadable(byName, out, p, p.kind, p.inherits).target) continue;
...
function firstLoadable(
  ...
  const ordered = loadOrder(inCollection);
  return { ordered, target: ordered.find((c) => !notLoaded.has(c.id)) };
}
 ...
 *                       nothing here.
 *  - `not-loaded`       it exists, in the folder the slicer *does* load, and the
 *                       slicer still skipped it — a `version` that does not parse,
 *                       or its own `inherits` failing. This is the one that reads
 *                       as "the file is right there", and `index.notLoaded` says
 *                       which gate it hit.
...
export function classifyReference(
  ...
  const { ordered, target } = firstLoadable(index.byName, index.notLoaded, from, targetKind, name);
  if (ordered.length === 0) {
